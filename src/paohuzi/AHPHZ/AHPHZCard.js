 ...
 */
var AHPHZCard = ccui.Widget.extend({
        ...
        if(type == ccui.Widget.TOUCH_BEGAN){
            ...
            if(this.y>=PHZSetModel.cardTouchend){//拖出去了松开，并且超过出牌线，直接出牌
                if(isMyTurn && PHZRoomModel.selfAct.length==0 && !this._cardVo.grayNoChu &&
                    !((PHZRoomModel.wanfa == PHZGameTypeModel.LDS || PHZRoomModel.wanfa == PHZGameTypeModel.YZCHZ) && this._cardVo.n == 11)){//落地扫王牌不能出
                    // this.setNormalColor();
                    PHZRoomModel.outCardsX = this.x;
                    PHZRoomModel.outCardsY = this.y;
                    PHZRoomModel.isSelfOutCard = true;
                    PHZRoomModel.simulateLetOutCard(this._cardVo.c,JSON.stringify(AHPHZMineLayout.curSortedData));
                }else{
                ...
                var isArrived = false;
                var isInsert = false;
                for(var i=0;i<AHPHZMineLayout.coords.length;i++){
                    var coord = AHPHZMineLayout.coords[i];
                    var tCardListLength = AHPHZMineLayout.curSortedData[i].length;
                    ...
                    if(Math.abs(this.x-coord.x)<67 && Math.abs(this.y-coord.y)<241){
                        ...
                        if(Math.abs(this.x-coord.x)<67 && Math.abs(this.y-coord.y)<241){
                            if(AHPHZMineLayout.curSortedData[i].length<=4){
                                this.setLocalZOrder(2 - insertIndexY );
                                AHPHZMineLayout.reSortAndDisplay(this._cardVo,i,isInsert,insertIndexY);
                                isArrived = true;
                            }
                ...
                if(!isArrived){
                    if(this.x>AHPHZMineLayout.coords[AHPHZMineLayout.coords.length-1].x+66 && AHPHZMineLayout.curSortedData.length<10){
                        AHPHZMineLayout.reSortAndDisplay(this._cardVo,-1,false);
                    }else if(this.x<AHPHZMineLayout.coords[0].x-66 && AHPHZMineLayout.curSortedData.length<10){
                        AHPHZMineLayout.reSortAndDisplay(this._cardVo,-2,false);
                        //}else{
                        //    this.setLocalZOrder(this.initZorder);
                        //    this.x = this.initX;
                        //    this.y = this.initY;
    ...
    diplay:function(action){
        ...
        if(an==1){
            ...
            if(place == 1){
                if (PHZSetModel.zpdx == 3 || PHZSetModel.zpdx == 4){
                    scale = 0.88;
                }else if (PHZSetModel.zpdx == 2 || PHZSetModel.zpdx == 1){
                    scale = 0.8;
                }
        ...
        if (this._cardVo.ishu){ //当前胡的这张牌
            scale = 0.45;
            bgPng = "big_half_face1_"+pmType+".png"
        }
        ...
        if(same==1&&direct==1&&place==1){//大于等于3个一样的 加一层遮罩
            if(PHZRoomModel.wanfa != PHZGameTypeModel.LDS && PHZRoomModel.wanfa != PHZGameTypeModel.YZCHZ){//落地扫不让加置灰，说是看着不爽，我了个去
                bgImg.setColor(cc.color(175,175,175));
            }
        }
