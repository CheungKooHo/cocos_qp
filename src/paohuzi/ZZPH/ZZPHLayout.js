 ...
 */
var ZZPHLayout = cc.Class.extend({
                ...
                for(var i=0;i<this.data2.length;i++){
                    ...
                    if( innerObject.action==PHZAction.WEI
                        ...
                        if(ArrayUtil.indexOf(ids,vo.c)>=0){
                            ...
                            if(innerObject.cards.length == 3){
                                this.assignInnerObjectByPao(innerObject,action,ids[3]);
                            }
                            hasInsert = true;
                            break;
                        }
    ...
    beiPengPai:function(id){
        ...
        var del = -1;
        for(var i=0;i<this.data3.length;i++){
            cc.log("this.data3[i].c =",this.data3[i].c);
    ...
    showPengLogo:function(data){
        for (var i = 0; i < this.mahjongs3.length; i++) {
            for (var j = 0; j < data.length; j++) {
                if(this.mahjongs3[i]._cardVo.c == data[j]){
                    this.mahjongs3[i].showPengLogo();
                }
            }
        }
    },
    ...
    refreshP2:function(data,isOver){
        ...
        for(var i=0;i<data.length;i++){
            ...
            for(var j=0;j<innerArray.length;j++){
                ...
                if(count<nowCards){
                    ...
                    card = new ZZPHCard(PHZAI.getDisplayVo(this.direct,2),innerVo,innerAction);
                    this.mPanel.addChild(card);
                    this.mahjongs2.push(card);
                }
                if(PHZRoomModel.renshu==3 && this.direct==2){
                    card.x = this.mPanel.width-((i+1)*gx);
                }else{
                    card.x = i*gx;
                }
        ...
        if(PHZRoomModel.renshu==3 && this.direct==2){
            this.place2x = (data.length+1)*gx - this.mPanel.width;
        }
    },
    ...
    refreshP3:function(data,isQiAni){
        ...
        if(PHZRoomModel.renshu==3){
            initVal=(this.direct==3) ? 168 : 0;
        }else if(PHZRoomModel.renshu==4){
            initVal=(this.direct==4) ? 168 : 0;
        }
        for(var i=0;i<this.mahjongs3.length;i++){
        ...
        for(;i<data.length;i++){
            ...
            //cc.log("i============="+i)
            var card = new ZZPHCard(PHZAI.getDisplayVo(this.direct,3),data[i]);
            ...
            if(PHZRoomModel.renshu==4){
                if(this.direct==4){
                    if(i<5){
                        ...
                        card.y = initVal-(i-5)*g;
                        card.x = 48*0.75;
                    }else{
                        card.y = initVal-(i-10)*g;
                        card.x = 96*0.75;
                    }
                    ...
                    if(i<6){
                        ...
                        card.x = initVal+(i-6)*g;
                        card.y = 48*0.75;
                    }else{
                        card.x = initVal+(i-12)*g;
                        card.y = 96*0.75;
                    }
                }
