 ...
 */
var ZZPHCard = ccui.Widget.extend({
    ...
    refreshCardBgByOpenTex:function() {
        if (this._bg && this.numberImg){
            ...
            var bgPng = "";
            if (place == 1){
                bgPng = PHZSetModel.zpdx == 1 ? "cards_small_face_"+pmType+".png" : "cards_small_face2_"+pmType+".png";
            }else{
                scale = 0.5;
                bgPng = "cards_small_face_"+pmType+".png";
            }
                
            if (this._cardVo.ishu){ //当前胡的这张牌
                scale = 0.45;
                bgPng = "cards_small_face1_"+pmType+".png"
            }
        ...
        if(type == ccui.Widget.TOUCH_BEGAN){
            ...
            if(this.y>=PHZSetModel.cardTouchend){//拖出去了松开，并且超过出牌线，直接出牌
                if(isMyTurn && PHZRoomModel.selfAct.length==0 && !this._cardVo.grayNoChu){//落地扫王牌不能出
                    ...
                    PHZRoomModel.simulateLetOutCard(this._cardVo.c,JSON.stringify(ZZPHMineLayout.curSortedData));
                }else{
                ...
                var isArrived = false;
                var isInsert = false;
                for(var i=0;i<ZZPHMineLayout.coords.length;i++){
                    var coord = ZZPHMineLayout.coords[i];
                    var tCardListLength = ZZPHMineLayout.curSortedData[i].length;
                    ...
                    if(Math.abs(this.x-coord.x)<67 && Math.abs(this.y-coord.y)<241){
                        ...
                        if(Math.abs(this.x-coord.x)<67 && Math.abs(this.y-coord.y)<241){
                            if(ZZPHMineLayout.curSortedData[i].length<=4){
                                this.setLocalZOrder(2 - insertIndexY );
                                ZZPHMineLayout.reSortAndDisplay(this._cardVo,i,isInsert,insertIndexY);
                                isArrived = true;
                            }
                ...
                if(!isArrived){
                    if(this.x>ZZPHMineLayout.coords[ZZPHMineLayout.coords.length-1].x+66 && ZZPHMineLayout.curSortedData.length<10){
                        ZZPHMineLayout.reSortAndDisplay(this._cardVo,-1,false);
                    }else if(this.x<ZZPHMineLayout.coords[0].x-66 && ZZPHMineLayout.curSortedData.length<10){
                        ZZPHMineLayout.reSortAndDisplay(this._cardVo,-2,false);
                        //}else{
                        //    this.setLocalZOrder(this.initZorder);
                        //    this.x = this.initX;
                        //    this.y = this.initY;
    ...
    diplay:function(action){
        ...
        var pengLogo  = this.pengLogo = new cc.Sprite("res/ui/phz/zzph/lan_small.png");
        pengLogo.x = bgImg.width*0.20;
        pengLogo.y = bgImg.height*0.75;
        pengLogo.setName("pengLogo");
        bgImg.addChild(pengLogo);
        pengLogo.visible = false;
        if(this._cardVo.isshowPengLogo){
            pengLogo.visible = true;
        }
    },

    showPengLogo:function(){
        this.pengLogo.visible = true;
    },
    /**
     * 获取数据模型
     * @returns {CardVo}
