 ...
 */
var XXEQSCard = ccui.Widget.extend({
        ...
        if(type == ccui.Widget.TOUCH_BEGAN){
            ...
            if(isMyTurn && PHZRoomModel.selfAct.length==0){
                if (PHZRoomModel.getTouchCard() != this._cardVo.v){
                    ...
                    if(PHZRoomModel.wanfa == PHZGameTypeModel.WHZ || PHZRoomModel.wanfa == PHZGameTypeModel.LDS
                        ...
                        || PHZRoomModel.wanfa == PHZGameTypeModel.YJGHZ){
            ...
            if(this.y>=PHZSetModel.cardTouchend){//拖出去了松开，并且超过出牌线，直接出牌
                if(isMyTurn && PHZRoomModel.selfAct.length==0 && !this._cardVo.grayNoChu &&
                    ...
                    || PHZRoomModel.wanfa == PHZGameTypeModel.GLZP) && this._cardVo.n == 11)){//落地扫王牌不能出
                    // this.setNormalColor();
    ...
    diplay:function(action){
        ...
        if(PHZRoomModel.wanfa == PHZGameTypeModel.WHZ || PHZRoomModel.wanfa == PHZGameTypeModel.YJGHZ){
            same = 0;
        }
        ...
        if(PHZRoomModel.wanfa == PHZGameTypeModel.YJGHZ && same == 1){
            same = 0;
            this.setGrayCanMove();
        }
        ...
        if(PHZRoomModel.wanfa == PHZGameTypeModel.XXEQS){
            if(this._cardVo.isshowPengLogo){
                this.addOutCardType(bgImg)
            }
        }

    },

    addOutCardType:function(card){
        var signImg  = this.tingImg = new cc.Sprite("res/ui/phz/zzph/lan_small.png");
        signImg.x = 11;
        signImg.y = 99;
        card.addChild(signImg);
    },
