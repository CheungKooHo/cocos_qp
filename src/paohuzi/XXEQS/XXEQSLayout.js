 ...
 */
var XXEQSLayout = cc.Class.extend({
    ...
    initData:function(direct,mPanel,oPanel,banker,isMoPai){
        ...
        if(SdkUtil.isIphoneX() && direct == 1 && PHZRoomModel.is3Ren()){
            this.mPanel.x = 45;
        }
        ...
        if(SdkUtil.isIphoneX() && direct == 1 && PHZRoomModel.is2Ren()){
            this.mPanel.x = 45;
        }

        this.chumopaiCount = 0;//出摸牌的数量
        this.chushoupaiCount = 0;//出手牌的数量

        if(!this.oPanel.getChildByName("chupaibeijing")) {
            var chupaibeijing;
            if (PHZRoomModel.is2Ren() && direct == 2 || PHZRoomModel.is3Ren() && direct == 3) {
                chupaibeijing = new cc.Scale9Sprite("res/ui/phz/xxeqs/chupaibeijing2.png", null, cc.rect(30, 30, 7, 7));
                chupaibeijing.setPosition(128, 28)
            } else {
                chupaibeijing = new cc.Scale9Sprite("res/ui/phz/xxeqs/chupaibeijing1.png", null, cc.rect(30, 30, 7, 7));
                chupaibeijing.setPosition(219, 28)
            }
            chupaibeijing.width = 264
            this.oPanel.addChild(chupaibeijing);
        }
    },
            ...
            if(action==PHZAction.TI||action==PHZAction.PAO){//先偎后提、跑
                for(var i=0;i<this.data2.length;i++){
                    ...
                    if( innerObject.action==PHZAction.WEI
                        ...
                        if(this.direct != 1 && isZaiPao == 1 && PHZRoomModel.wanfa == PHZGameTypeModel.NXPHZ && innerObject.action==PHZAction.WEI){
                            if(vo.c<=0){
                        ...
                        if(this.direct != 1 && isZaiPao == 1 && PHZRoomModel.wanfa == PHZGameTypeModel.SMPHZ && innerObject.action==PHZAction.WEI){
                            if(vo.c<=0){
                                this.assignInnerObjectByPao(innerObject,action,ids[0]);
                                hasInsert = true;
                                break;
    ...
    refresh:function(data1,data2,data3,data4){
        if(PHZRoomModel.wanfa == PHZGameTypeModel.XXEQS) {
            this.outCardTypes = data3.concat(0)
        ...
        if(PHZRoomModel.wanfa == PHZGameTypeModel.XXEQS) {
            for (var i = 0; i < this.outCardTypes.length; i++) {
                if (this.outCardTypes[i] >= 1000) {
                    data3[i].isshowPengLogo = true;
                }
            }
        }
        this.data2 = data2;
            ...
            for(var j=0;j<innerArray.length;j++){
                ...
                switch (innerAction){
                    case PHZAction.WEI://偎
                        ...
                        if(PHZRoomModel.wanfa == PHZGameTypeModel.LDS || PHZRoomModel.wanfa == PHZGameTypeModel.YZCHZ){//落地扫偎牌别人全暗，自己全显示暗一点
                    ...
                    case 18://岳阳歪胡子的起手4张溜，全部扑牌不显示
                        ...
                        if(j<3)
                        	innerVo.a=1;
                        if(innerAction == 18 && PHZRoomModel.wanfa == PHZGameTypeModel.WHZ)innerVo.a = 1;
                        if(PHZRoomModel.wanfa == PHZGameTypeModel.LDS || PHZRoomModel.wanfa == PHZGameTypeModel.YZCHZ){//落地扫提牌牌别人全暗，自己全显示暗一点
                            if(this.direct != 1 && !isOver)innerVo.a = 1;
                            else {
                                innerVo.a = 0;
                                innerVo.zhe = 1;
                ...
                if(count<nowCards){
                	...
                	card = new XXEQSCard(PHZAI.getDisplayVo(this.direct,2),innerVo,innerAction);
    ...
    refreshP3:function(data,isQiAni){
        ...
        var twoRenLineMax = 15
        var threeRenLineMax = 6
        for(;i<data.length;i++){
            //cc.log("i============="+i)
            var card = new XXEQSCard(PHZAI.getDisplayVo(this.direct,3),data[i]);

            this.mahjongs3.push(card);
            if(PHZRoomModel.renshu==3){
                if(this.direct==3){
                    if(data[i].isshowPengLogo){
                        card.x = initVal+this.chushoupaiCount*g;
                        this.chushoupaiCount++;
                    }else{
                        if(this.chumopaiCount < threeRenLineMax){
                            card.x = initVal+this.chumopaiCount*g;
                            card.y = 60;
                        }else{
                            card.x = initVal+(this.chumopaiCount-threeRenLineMax)*g;
                            card.y = 120;
                        }
                        this.chumopaiCount++;
                    }
                ...
                if(this.direct==2){
                    if(data[i].isshowPengLogo){
                        ...
                        if(this.chumopaiCount < twoRenLineMax){
                            card.x = initVal+this.chumopaiCount*g;
                            card.y = 60;
                        }else{
                            card.x = initVal+(this.chumopaiCount-twoRenLineMax)*g;
                            card.y = 120;
                        }
                        this.chumopaiCount++;
                    }
            ...
            //cc.log("chumopaiCount",this.chushoupaiCount,this.chumopaiCount)
