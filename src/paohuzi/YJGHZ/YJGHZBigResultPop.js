 ...
 */
var YJGHZBigResultPop = BasePopup.extend({
    user:null,
    isDaiKai:null,
    ...
    refreshSingle:function(widget,user){
        ...
        typeData.push({name:"胡牌次数",num:(user.winCount || 0)});
        typeData.push({name:"大胡次数",num:dahuNum});

        if(PHZRoomModel.wanfa == PHZGameTypeModel.YJGHZ){
            for(var i = 0;i<countsArr.length;++i){
                if(countsArr[i] > 0){
                    typeData.push({name:dahuName[i],num:countsArr[i]});
                }
            }
        }
    ...
    selfRender: function () {
        ...
        if(PHZRoomModel.wanfa == PHZGameTypeModel.YJGHZ){
            this.getWidget("ext2").setString("沅江鬼胡子");
        }else if(PHZRoomModel.wanfa == PHZGameTypeModel.NXGHZ){
