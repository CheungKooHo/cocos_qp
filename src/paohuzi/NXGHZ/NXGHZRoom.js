...
var NXGHZRoom = BaseLayer.extend({ //BaseLayer BaseRoom
    layouts:{},
    tag_btn_chi:601,
    tag_btn_liu:602,
    tag_btn_other:501,
    ...
    initwanfaImg:function(){
        ...
        if (this.Panel_20.getChildByName("wanfaImg")){
            ...
            wanfa_bg.setPosition(this.button_wanfa.x,this.button_wanfa.y - wanfa_bg.height/2-20);
            ...
            bg.setPosition(this.button_wanfa.x,this.button_wanfa.y - bg.height/2-20);
            bg.setName("wanfaImg");
            bg.visible = false;
            this.Panel_20.addChild(bg,201);
            var wanfa_label = new cc.LabelTTF("","Arial",22,cc.size(300, bg.height));
            ...
            wanfa_label.setPosition(bg.width/2,bg.height/2 - 10);
        }
        ...
        if (this.layouts && this.layouts[1]) {
            ...
            if (arr && arr.length > 0) {
                for (var i = 0; i < arr.length; i++) {
                    ...
                    if( innerObject.action==NXGHZAction.WEI || innerObject.action==NXGHZAction.PENG){
                        if (cards.v == innerObject.cards[0].v) {
                            ...
                            if (this.lastLetOutSeat == PHZRoomModel.mySeat && innerObject.action==NXGHZAction.WEI){
                                this.isSelfMo = true;
                            }
                            //减去当前操作的牌的胡息
                            var huxi = 0;
                            if (innerObject.action==NXGHZAction.PENG){
                                if (cards.v > 100){
                                    huxi = 3;
                                }else{
                                    huxi = 1;
                                }
                            }else if(innerObject.action==NXGHZAction.WEI){
                                if (cards.v > 100){
                                    huxi = 6;
                                }else{
                                    huxi = 3;
...
/*        var mc = new ChatPop("res/phzChat.json");
    ...
    onOver:function(event){
        ...
        this.showResultTimeOutHandle = setTimeout(function(){//延迟弹出结算框
            ...
            var mc = new NXGHZSmallResultPop(data);
    ...
    initData:function(){
        ...
        for(var i=0;i<players.length;i++){
            ...
            if(!isContinue){
                ...
                this._players[p.seat].updateHuXi(layout.huxi)
    ...
    onPengPai:function(obj,isAlert){
        ...
        switch (temp){
            case NXGHZAction.HU:
            case NXGHZAction.HU_JDB:
                PHZRoomModel.sendPlayCardMsg(temp,[]);
                break;
            case NXGHZAction.PENG:
            ...
            case NXGHZAction.CHI:
                ...
                break;
            case NXGHZAction.PAO:
                PHZRoomModel.sendPlayCardMsg(NXGHZAction.PAO,[this.lastLetOutMJ]);
                break;
            case NXGHZAction.WEI:
                PHZRoomModel.sendPlayCardMsg(temp,[]);
                break;
            case NXGHZAction.TI:
                ...
                if(result.length > 1){
                    ...
                    PHZRoomModel.sendPlayCardMsg(NXGHZAction.TI,data);
                }
                break;
        }
    },
    ...
    chooseChi:function(){
        ...
        for(var i=0;i<data2.length;i++){
            if(data2[i].action == NXGHZAction.CHI){
    ...
    getLiuCards:function(){
        ...
        if(this.lastLetOutMsg && this.lastLetOutMsg.actType == 0
            && this.lastLetOutMsg.action != NXGHZAction.GUO){
            tempCards = {};
        }
        ...
        for (var i = 0; i < moldCards.length; ++i) {
            ...
            if (moldCards[i].action == NXGHZAction.WEI) {
    ...
    onSelectLiuCard:function(sender){
        ...
        PHZRoomModel.sendPlayCardMsg(NXGHZAction.TI,sender.passArray);
    },
    ...
    startGame:function(event){
        ...
        for(var i=1;i<=PHZRoomModel.renshu;i++){
            ...
            if(layout)//清理掉上一次的牌局数据
                layout.clean();
            this._players[i].huxi = 0
            this._players[i].updateHuXi(0)
        }
    ...
    onLetOutCard:function(event){
        ...
        if(action==NXGHZAction.CHI && seat == PHZRoomModel.mySeat && ids.length>0){
        ...
        if(action == NXGHZAction.GUO || action == NXGHZAction.HU)isCleanChuPai = false;
        //庄家第一手出牌，闲家提，不清理出牌
        if(this.lastLetOutSeat>0&&action==NXGHZAction.TI && seat!=this.lastLetOutSeat)isCleanChuPai = false;

        isCleanChuPai && this.cleanChuPai();


        //系统摸的牌，在下次有出牌动作时再放到出牌的位置
        if(this.lastMoPHZ>0 && this.lastLetOutSeat>0 && (actType!=0 || (actType==0&&(action==NXGHZAction.TI || action==NXGHZAction.LIU_WEI)))) {
            this.layouts[PHZRoomModel.getPlayerSeq("", this.lastLetOutSeat)].chuPai(PHZAI.getPHZDef(this.lastMoPHZ));
        }

        if((actType==0&&(action==NXGHZAction.TI || action==NXGHZAction.LIU_WEI))) {
    ...
    onLetOutPai:function(message){
        ...
        var self = this;
        if(actType==0&&action==NXGHZAction.GUO){
            //noting to do
            if(seat==PHZRoomModel.mySeat)
        ...
        if(action != NXGHZAction.GUO && seat == PHZRoomModel.mySeat){
            PHZRoomModel.cleanCurrentlyChiData();//如果不是吃的操作时，要清除当前吃的数据
        }

        //手上如果有碰或者偎的那张牌 那张牌不能被打出去
        if((action==NXGHZAction.PENG || action == NXGHZAction.WEI) && seat==PHZRoomModel.mySeat) {
                ...
                if(ids.length>0){
                    ...
                    PHZRoomEffects.chiAnimate(ids, this.root, direct, function () {
                        ...
                        if (nowBurCount == PHZRoomModel.nowBurCount) {
                            ...
                            self._players[seat].updateHuXi(self.layouts[direct].huxi)
    ...
    showJianTou:function(seat,isTing){
        ...
        //cc.log("PHZRoomModel.isShowFinger()",JSON.stringify(PHZRoomModel.isShowFinger()))
    ...
    checkGrayCard:function(){
        if(PHZRoomModel.wanfa == PHZGameTypeModel.NXGHZ){
            ...
            if(moldCards){
                ...
                for(var i = 0;i< moldCards.length;++i){
                    var cards = moldCards[i].cards;
                    if(moldCards[i].action == NXGHZAction.CHI){
                    ...
                    }else if(moldCards[i].action == NXGHZAction.PENG || moldCards[i].action == NXGHZAction.WEI){
    ...
    updateBgColor:function(){
        ...
        var wanfaUrl = "";
        if (PHZRoomModel.wanfa == PHZGameTypeModel.NXGHZ){
            gameTypeUrl = "res/ui/phz/pp/gametype1_nxghz.png";
        }else{
        ...
        if (PHZSetModel.zmbj == 1){
            ...
            if (PHZRoomModel.wanfa == PHZGameTypeModel.NXGHZ){
                gameTypeUrl = "res/ui/phz/pp/gametype2_nxghz.png";
            }
            ...
            if (PHZRoomModel.wanfa == PHZGameTypeModel.NXGHZ){
                gameTypeUrl = "res/ui/phz/pp/gametype3_nxghz.png";
            }
