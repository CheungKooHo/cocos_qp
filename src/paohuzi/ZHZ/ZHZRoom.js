 ...
 */
var ZHZRoom = BaseLayer.extend({ //BaseLayer BaseRoom
    layouts:{},
    tag_btn_chi:601,
    tag_btn_other:501,
    tag_chi_select:343,
...
/*        var mc = new ChatPop("res/phzChat.json");
        ...
        for(var i=0;i<players.length;i++){
            ...
            if(PHZRoomModel.wanfa == PHZGameTypeModel.LDFPF){
                ...
                if(p.ext[8] != -1){
                    ...
                    // cc.log("PHZRoomModel.intParams[18] =",PHZRoomModel.intParams[18]);
                    if (p.ext[12]!=-1){
    ...
    displayChiSelect:function(cardsArray,result,needTimes,curTime){
        ...
        for(var i=0;i<result.length;i++){
            ...
            for(var j=0;j<array.length;j++){
                var phz = new ZHZCard(ZHZAI.getDisplayVo(1,2),array[j]);
                var scale = 0.5;
                // phz.scale=scale;
        ...
        bg.y = 240;
        cc.log("bg.x =",bg.x);
        cc.log("this.btnPanel.getChildByTag(this.tag_btn_chi).x =",this.btnPanel.getChildByTag(this.tag_btn_chi).x);
    ...
    onLetOutCard:function(event){
        ...
        if(lastMoPHZ>0 && this.lastLetOutSeat>0 && (actType!=0 || (actType==0&&action==ZHZAction.TI))) {
            var notTi = true;
            if (actType==0&&action==ZHZAction.TI){
                notTi = false;
            }
            this.layouts[PHZRoomModel.getPlayerSeq("", this.lastLetOutSeat)].chuPai(ZHZAI.getPHZDef(lastMoPHZ),notTi);
            if (notTi){
                isOutCard = false;
    ...
    onInvite:function(){
        ...
        obj.description=clubStr + csvhelper.strFormat("捉红字，{0}局，{1}",PHZRoomModel.totalBurCount ,wanfaDesc);
        obj.shareType=1;
        //SdkUtil.sdkFeed(obj);
        ShareDTPop.show(obj);
    },
