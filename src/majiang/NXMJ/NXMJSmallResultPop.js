 ...
 */
var NXMJSmallResultCell = ccui.Widget.extend({
...
var NXMJSmallResultPop = BasePopup.extend({
    ...
    createMoldPais: function(widget,user) {
        ...
        for (var i=0;i<moldPais.length;i++) {
            ...
            for(var j=0;j<innerArray.length;j++){
                ...
                var card = new NXMahjong(MJAI.getDisplayVo(1,2),innerVo);
                ...
                if(gangVo && j==1){
                    if(!card.getChildByTag(333)){
                        var gang = new NXMahjong(MJAI.getDisplayVo(1,2),gangVo);
                        gang.y += 12;
                        gang.scale = 1;
    ...
    createHandPais: function(widget,user) {
        ...
        for (var i=0;i<voArray.length;i++) {
            ...
            if (MJRoomModel.wanfa == MJWanfaType.TJMJ){
                if (MJRoomModel.tjmj_dipaiID && MJRoomModel.tjmj_tianpaiID){
                    if (voArray[i].t ==  MJAI.getMJDef(MJRoomModel.tjmj_tianpaiID).t && (voArray[i].n == first_wang)){
                        voArray[i].wang = 1;
                    }
                }
            }else if (MJRoomModel.wanfa == MJWanfaType.GDCSMJ){
            ...
            var card = new NXMahjong(MJAI.getDisplayVo(1,1),voArray[i]);
            var size = card.getContentSize();
            var _scale = 0.6;
            card.scale = _scale;
        ...
        if(MJRoomModel.wanfa == MJWanfaType.TJMJ){
            if(tjmjHuPai){
                var card = new NXMahjong(MJAI.getDisplayVo(1,1),tjmjHuPai);
                var size = card.getContentSize();
                var _scale = 0.6;
                card.scale = _scale;
    ...
    createHuedPais: function(widget,user) {
        ...
        var dahuConfig = {0:"碰碰胡",1:"将将胡",2:"清一色",3:"海底捞月",4:"海底炮",5:"七小对",
            6:"豪华七小对",7:"杠上开花",8:"抢杠胡",9:"杠上炮",10:"全求人",11:"双豪华七小对",12:"天胡",13:"地胡",14:"门清",15:"报听"};
        ...
        if(MJRoomModel.wanfa == MJWanfaType.NXMJ){
            if(user.ext[1] > 0){
    ...
    createHuCell: function(huRecord) {
        ...
        return new NXMJSmallResultCell(huUser, paoUser, ext);
    },
    ...
    onShowMoreResult:function(){
        var mc = new NXMJSmallResultOtherPop(this.data);
    ...
    showJiangma:function(){//奖马
        ...
        if(reasonA || reasonB || reasonC){
            ...
            for(var i = 0;i < mapai.length;++i){
                ...
                var card = new NXMahjong(MJAI.getDisplayVo(1, 3), mj);
                card.scale = 0.6;
                card.x = 1080 - 30 * i;
                card.y = 15;
                widget.addChild(card);
    ...
    showBirds: function() {
        ...
        if (birdList){
            for(var j=0;j<birdList.length;j++) {
                var id = birdList[j];
                var mj = MJAI.getMJDef(id);
                var card = new NXMahjong(MJAI.getDisplayVo(1, 2), mj);
                var size = card.getContentSize();
                var _scale = 0.7;
                card.scale = _scale;
    ...
    onOk:function(){
        ...
        if(MJRoomModel.nowBurCount == MJRoomModel.totalBurCount
            || (MJRoomModel.wanfa == MJWanfaType.NXMJ && data.ext[24] == 1)
            ...
            PopupManager.remove(this);
            var mc = new NXMJBigResultPop(data);
            PopupManager.addPopup(mc);
        }else{
            this.issent = true;
