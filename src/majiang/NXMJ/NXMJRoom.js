...
var NXMJRoom = BaseRoom.extend({
    ...
    selfRender:function(){
        ...
        //this.addCustomEvent(SyEvent.NXMJ_Ting_STATE, this,this.onTingClick);
        ...
        var roomFile = "res/ui/mj/nxmjRoom/nxmj_title.png";
        //if(MJRoomModel.wanfa == MJWanfaType.TDH){
    ...
    onTingClick:function(event){
        ...
        if(message.action == 24){
            MJRoomSound.actionSound(userId,"baoting");
            if (p.seat == MJRoomModel.mySeat){
                MJRoomModel.tjmjTing = true;//听牌玩家锁牌
            }
        }
        this._players[p.seat].setTingState(message.action == 24);
    },
    ...
    showPiaoBtn:function(isShow){
        if(isShow){
            if(!this.piaoBtnNode){
                ...
                var imgArr = [{tag:0,img:"bupiao.png"},{tag:1,img:"1fen.png"},{tag:2,img:"2fen.png"},{tag:4,img:"4fen.png"}];
                var offsetX = 170;
                var startX = -(imgArr.length - 1)/2*offsetX;
                for(var i = 0;i<imgArr.length;++i){
                    var img = "res/ui/mj/csmjRoom/" + imgArr[i].img;
                    var btn = new ccui.Button(img);
                    btn.setTag(imgArr[i].tag);
                    btn.setScale(0.6);
    ...
    onXiaoHuPlayCard:function(){
        ...
        //    if(this.tjmjTingData){//如果有听牌
        //        SyEventManager.dispatchEvent(SyEvent.DAPAI_TING,{info:this.tjmjTingData});
        //    }
        //}
        var self = this;
    ...
    showTJMJGang:function(seat){
        ...
        if(acts.length > 0){
            ...
            for(var i = 0;i<acts.length;++i){
                ...
                var card = new NXMahjong(MJAI.getDisplayVo(1, 1), MJAI.getMJDef(id));
                card.x = i * 105 + 10;
                card.y = 5;
            ...
            this.TJMJGang_Panel.width = 5+acts.length*105;
    ...
    onCSGang:function(event){
        ...
        //桃江麻将
        if(MJRoomModel.wanfa == MJWanfaType.TJMJ || MJRoomModel.wanfa == MJWanfaType.TDH|| MJRoomModel.wanfa == MJWanfaType.NXMJ){
    ...
    csGangMoPai:function(seat){
        ...
        if(seat == MJRoomModel.mySeat){
            ...
            for(var i = 0;i<playIds.length;++i){
                ...
                if(MJRoomModel.wanfa == MJWanfaType.TDH || MJRoomModel.wanfa == MJWanfaType.NXMJ){
                    delay += 1.2;
                }else{
                    delay += 0.5;
                }
    ...
    outCardTing:function(event){
        if(MJRoomModel.wanfa == MJWanfaType.TJMJ && (MJRoomModel.selfAct[0] == 1 || MJRoomModel.selfAct[17] == 1)){
    ...
    onShowHuPanel:function(isShowAllTime){
        ...
        if (huList && huList.length > 0){
            ...
            for (var i = 0; i < huList.length; i++) {
                ...
                var card = new NXMahjong(MJAI.getDisplayVo(1, 2), vo);
    ...
    onShowHuCardsByTingPai:function(event){
        ...
        if(huCards.length>0) {
            ...
            for (var i = 0; i < huCards.length; i++) {
                var vo = huCards[i];
                var card = new NXMahjong(MJAI.getDisplayVo(1, 4), vo);
                card.x = orderNum * 27;
                card.y = 0;
    ...
    onCheckResult:function(){
        if(this.resultData){
            var mc = new NXMJSmallResultPop(this.resultData);
            PopupManager.addPopup(mc);
        }
    },
    ...
    onJixuFromResult:function(){
        this.Panel_8.visible = false;
        if(MJRoomModel.totalBurCount == MJRoomModel.nowBurCount  || (MJRoomModel.wanfa == MJWanfaType.NXMJ && ClosingInfoModel.ext[24] == 1)
            ...
            var mc = new NXMJBigResultPop(this.resultData);
            PopupManager.addPopup(mc);
        }else {
    ...
    onOver:function(event){
        ...
        this.overTimeout = setTimeout(function(){//延迟弹出结算框
            ...
            var mc = new NXMJSmallResultPop(data);
            PopupManager.addPopup(mc);
        ...
        for(var i=0;i<players.length;i++){
            ...
            if(p.ext.length>0){//长春麻将，听牌了
                ...
                if(p.ext.length>1 && p.ext[1]===1)//是否摸牌
                    isMoPai = true;
                this._players[p.seat].setTingState(p.ext[7] == 1);//报听后的听牌图标
            ...
            var point = -1;
            if((MJRoomModel.wanfa == MJWanfaType.NXMJ && MJRoomModel.ext[15] == 1)
            ...
            if(!isContinue){
                ...
                this.showDiCard(MJRoomModel.ext[18],MJRoomModel.ext[19],maArr,MJRoomModel.ext[20]);
    ...
    showNiaoPanel:function(data){
        ...
        for(var i = 0;i<birdData.length;++i){
            ...
            for(var j = 0;j<dirBird.length;++j){
                ...
                var card = new NXMahjong(MJAI.getDisplayVo(1, 1), vo);
    ...
    onShowHuCards:function(huArray){
        ...
        if(huArray.length>0){
            ...
            for(var key in huArray){
                var vo = huArray[key];
                //vo.tingDisplay = 1;
                var card = new NXMahjong(MJAI.getDisplayVo(1,4),huArray[key]);
                card.x = orderNum*27;
                card.y = 0;
                this.Panel_ting.addChild(card);
                orderNum += 1;
    ...
    displaySelectMahjongs:function(action,resultArray){
        ...
        for(var i=0;i<resultArray.length;i++){
            ...
            for(var j=0;j<chiArr.length;j++){
                ...
                var mahjong = new NXMahjong(MJAI.getDisplayVo(1,3),chiVo);
                mahjong.scale=scale;
    ...
    onOperate:function(obj,isAlert){
        ...
        var temp = obj.temp;
        switch (temp){
            case MJAction.NXMJBAO_TING:
                MJRoomModel.sendPlayCardMsg(MJAction.NXMJBAO_TING,[]);
                break;
            ...
            case MJAction.BU_ZHANG:
                var optFunc = function(){
                    ...
                    if(allMJs.length%3==2){//摸牌了，手上的牌和摊开的牌都需要找一遍
                        ...
                        if(temp == MJAction.GANG && MJRoomModel.wanfa == MJWanfaType.TJMJ){
                            if(result.length>1){
    ...
    showDiCard:function(diCardId,kingCardId,horses,index){
        if(MJRoomModel.wanfa == MJWanfaType.TJMJ){
    ...
    showMaima:function(horses,isBack,isXJ){
        if(horses && horses.length > 0){//买马
            ...
            var direct = 3;
            this.gdcsmjMaPai = new NXMahjong(MJAI.getDisplayVo(1, direct), vo);
            ...
            if(horses.length === 2){
                ...
                vo.a = isBack ? 1 : 0;
                var cards = new NXMahjong(MJAI.getDisplayVo(1, direct), vo);
                cards.y = 0;
                cards.x = 60;
            ...
            for(var i = 0;i < MJRoomModel.players.length;++i){
                if(MJRoomModel.players[i].seat != MJRoomModel.mySeat){
                    ...
                    for(var j = 0;j < temp.length;++j){
                        ...
                        vo.a = isBack ? 1 : 0;
                        cards = new NXMahjong(MJAI.getDisplayVo(place, direct), vo);
    ...
    showLiangWangAni:function(id,index){
        ...
        var scale = 0.6;
        this.wang_card = new NXMahjong(MJAI.getDisplayVo(1, 1), vo);
        this.wang_card.y = 360;
        this.wang_card.x = 705;
        ...
        if(MJRoomModel.wanfa == MJWanfaType.TJMJ){
            ...
            otherVo.wang = 1;
            var wangpai = new NXMahjong(MJAI.getDisplayVo(1, 1), otherVo);
            wangpai.x = 135;
    ...
    refreshButton:function(selfAct){
        ...
        if(selfAct.length>0){
            ...
            var textureMap = {
                ...
                5:{t:"res/ui/mj/zzmjRoom/mj_btn_bu.png",v:7},
                6:{t:"res/ui/mj/csmjRoom/qys.png",v:8},
                7:{t:"res/ui/mj/csmjRoom/bbh.png",v:8},
                8:{t:"res/ui/mj/csmjRoom/yzh.png",v:8},
                9:{t:"res/ui/mj/csmjRoom/llx.png",v:8},
                10:{t:"res/ui/mj/csmjRoom/dsx.png",v:8},
                11:{t:"res/ui/mj/csmjRoom/jtyy.png",v:8},
                12:{t:"res/ui/mj/csmjRoom/jjg.png",v:8},
                13:{t:"res/ui/mj/csmjRoom/st.png",v:8},
                14:{t:"res/ui/mj/csmjRoom/ztsx.png",v:8},
                15:{t:"res/ui/mj/csmjRoom/ztllx.png",v:8},
                16:{t:"res/ui/mj/zzmjRoom/mj_btn_bu.png",v:7,v1:14},
                17:{t:"res/ui/mj/zzmjRoom/mj_btn_zimo.png",v:1},
                18:{t:"res/ui/mj/nxmjRoom/nxmj_btn_ting.png",v:25},
                19:{t:"res/ui/mj/nxmjRoom/nxmj_ydh.png",v:19}
            };
            if(MJRoomModel.wanfa == MJWanfaType.TJMJ){
    ...
    onLetOutCard:function(event){
        this.updateCountDown(this.COUNT_DOWN);
        var message = event.getUserData();
        cc.log("onLetOutCard",JSON.stringify(message))
        var userId = message.userId;
        var seat = message.seat;
        ...
        switch (action) {
            ...
            case MJAction.CHU_PAI://出牌动作
                ...
                if(ids.length > 0 && MJRoomModel.wanfa == MJWanfaType.TJMJ){
                    this.clearTingArrows();
                }
                break;
    ...
    smartFitlerMayHuPai:function(isReconect){
        ...
        if(list.length>0){
            ...
            for(var key in list){
                var vo = list[key];
                //vo.tingDisplay = 1;
                var card = new NXMahjong(MJAI.getDisplayVo(1,4),list[key]);
                card.x = orderNum*27;
                card.y = 0;
                this.Panel_ting.addChild(card);
                orderNum += 1;
