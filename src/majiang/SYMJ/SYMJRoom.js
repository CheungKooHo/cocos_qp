...
var SYMJRoom = BaseRoom.extend({
    ...
    ctor:function(json){
        ...
        if (MJRoomModel.isOpenTuoguan()){
            this.COUNT_DOWN = MJRoomModel.intParams[8];//90;
        }
        this._super(json);
    },
    ...
    selfRender:function(){
        ...
        this.Panel_jiachui = this.getWidget("Panel_jiachui");//加锤操作显示
        this.Panel_jiachui.visible = false;

        for (var i = 1; i <= 2; i++) {
            var btn = this.getWidget("Button_chui"+i);//选择加锤按钮
            btn.temp = i;
            UITools.addClickEvent(btn, this, this.onChooseDaChui);
        }
        ...
        /** SyEvent 添加加锤事件 */
        this.addCustomEvent(SyEvent.SYMJ_JIACHUI , this,this.showDaChui);
        this.addCustomEvent(SyEvent.SYMJ_JIACHUI_STATE , this,this.chooseDaChui);
    ...
    onChooseDaChui:function(obj){
        var temp = obj.temp - 1;
        sySocket.sendComReqMsg(215,[temp]);
    },


    showDaChui:function(event){
        this.Panel_jiachui.visible = true;
    },

    chooseDaChui:function(event){
        var message = event.getUserData();
        var params = message.params;
        //params[0]=seat座位号
        //params[1]=锤的值
        var seat = params[0];
        var doubleNum = params[1];
        //var p = MJRoomModel.getPlayerVoBySeat(seat);
        this.Panel_jiachui.visible = false;
        ////cc.log("chooseDouble===",seat,MJRoomModel.mySeat);
        //if (seat == 1 && !doubleNum && doubleNum != 0){
        //    this.Panel_jiachui.visible = true;
        //}
        //var isShow = false;
        if (this._players[seat]){//doubleNum && doubleNum > 0 &&
            //isShow = true;
            this._players[seat].showJaiChuiImg(doubleNum);
            //if (this._players[seat]){
            //    this._players[seat].showJaiChuiImg(isShow);
            //}
        }
    ...
    onShowHuPanel:function(isShowAllTime){
        ...
        if (huList && huList.length > 0){
            ...
            for (var i = 0; i < huList.length; i++) {
                ...
                var card = new SYMahjong(MJAI.getDisplayVo(1, 2), vo);
    ...
    onShowHuCardsByTingPai:function(event){
        ...
        if(huCards.length>0) {
            ...
            for (var i = 0; i < huCards.length; i++) {
                var vo = huCards[i];
                var card = new SYMahjong(MJAI.getDisplayVo(1, 4), vo);
                card.x = orderNum * 27;
                card.y = 0;
    ...
    onCheckResult:function(){
        if(this.resultData){
            var mc = new SYMJSmallResultPop(this.resultData);
            PopupManager.addPopup(mc);
        }
    },
    ...
    onJixuFromResult:function(){
        this.Panel_8.visible = false;
        if(MJRoomModel.totalBurCount == MJRoomModel.nowBurCount || ClosingInfoModel.ext[20]== 1){
            var mc = new SYMJBigResultPop(this.resultData);
            PopupManager.addPopup(mc);
        }else {
    ...
    onOver:function(event){
        ...
        for(var i=0;i<closingPlayers.length;i++){
            ...
            self._players[p.seat].hideJaiChuiImg();
        ...
        this.overTimeout = setTimeout(function(){//延迟弹出结算框
            ...
            var mc = new SYMJSmallResultPop(data);
            PopupManager.addPopup(mc);
    ...
    initData:function(){
        ...
        for(var i=0;i<players.length;i++){
            ...
            if(p.userId ==PlayerModel.userId){//自己的状态处理
                ...
                if (MJRoomModel.getIsJiaChui() && MJRoomModel.getIsJiaChuiState() == 1 && p.ext[3] == -1){
                    isShowjiaChui = true;
                }
            }
            cardPlayer.updateTuoguan(isTuoguan);
        }
        this.Panel_jiachui.visible = isShowjiaChui;
        //IP相同的显示
        if(players.length>1){
    ...
    onShowHuCards:function(huArray){
        ...
        if(huArray.length>0){
            ...
            for(var key in huArray){
                var vo = huArray[key];
                //vo.tingDisplay = 1;
                var card = new SYMahjong(MJAI.getDisplayVo(1,4),huArray[key]);
                card.x = orderNum*27;
                card.y = 0;
                this.Panel_ting.addChild(card);
                orderNum += 1;
    ...
    displaySelectMahjongs:function(action,resultArray){
        ...
        for(var i=0;i<resultArray.length;i++){
            ...
            for(var j=0;j<chiArr.length;j++){
                ...
                var mahjong = new SYMahjong(MJAI.getDisplayVo(1,3),chiVo);
                mahjong.scale=scale;
    ...
    onOperate:function(obj,isAlert){
        ...
        switch (temp){
            ...
            case MJAction.CHI:
                var lastVo = MJAI.getMJDef(this.lastLetOutMJ);
                //cc.log("lastVo",JSON.stringify(lastVo))
    ...
    smartFitlerMayHuPai:function(isReconect){
    	...
    	if(list.length>0){
            ...
            for(var key in list){
                var vo = list[key];
                //vo.tingDisplay = 1;
                var card = new SYMahjong(MJAI.getDisplayVo(1,4),list[key]);
                card.x = orderNum*27;
                card.y = 0;
                this.Panel_ting.addChild(card);
                orderNum += 1;
