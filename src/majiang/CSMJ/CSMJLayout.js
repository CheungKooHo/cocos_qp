...
var CSMJLayout = cc.Class.extend({
	gangPai:[],
     ...
     * @param ids
     */
    pengPai:function(ids,action,fromSeat,moreSeat){
            ...
            for(var i=0;i<this.data2.length;i++){
                ...
                if(innerObj.action==MJAction.PENG){
                    ...
                    if(curVo0.t==vo0.t&&curVo0.n==vo0.n){
                        ...
                        if(MJRoomModel.wanfa == MJWanfaType.GDCSMJ && action==MJAction.GANG){//广东潮汕麻将，明杠箭头指向自己
                            innerObj.huxi = moreSeat;
                        }
                        break;
                    }
                }
    ...
    getPlace2GangData:function(){
        var nArray = [];
        for(var i=0;i<this.data2.length;i++){
            var innerObject = this.data2[i];
            if(innerObject.action == MJAction.PENG){
                for(var j=0;j<innerObject.cards.length;j++){
                    nArray.push(innerObject.cards[j].c);
                }
            }
        }
        return nArray;
    ...
    refreshP1:function(data){
        var first_wang = 0;
        var second_wang = 0;
        if (MJRoomModel.wanfa == MJWanfaType.AHMJ){
            ...
            if (MJRoomModel.tcmj_dipaiID && MJRoomModel.tcmj_tianpaiID){
                var cardVo = MJAI.getMJDef(MJRoomModel.tcmj_tianpaiID);
                first_wang = cardVo;
            }
        ...
        for(var i=0;i<data.length;i++){
            if (MJRoomModel.wanfa == MJWanfaType.AHMJ){
                ...
                if (MJRoomModel.tjmj_dipaiID && MJRoomModel.tjmj_tianpaiID){
                    if (data[i].t ==  MJAI.getMJDef(MJRoomModel.tjmj_tianpaiID).t && (data[i].n == first_wang || data[i].n == second_wang)){
                        data[i].wang = 1;
                    }
                }
                ...
                if (MJRoomModel.tcmj_dipaiID && MJRoomModel.tcmj_tianpaiID){
                    if (data[i].t ==  MJAI.getMJDef(MJRoomModel.tcmj_tianpaiID).t && (data[i].i == first_wang.i)){
                        data[i].wang = 1;
                    }
                }
            }else if (MJRoomModel.wanfa == MJWanfaType.NYMJ){
                if (MJRoomModel.nymj_tianpaiID){
                    if (data[i].t ==  MJAI.getMJDef(MJRoomModel.nymj_tianpaiID).t && (data[i].i == first_wang.i)){
                        data[i].wang = 1;
                    }
                }
            }
        ...
        for(var j=0;j<data.length;j++){
            if (data[j].wang == 1 && data[j].m != 1){
                newData.push(data[j]);
            }
        }
        for(var j=0;j<data.length;j++){
            if (data[j].m == 1 || data[j].wang != 1){
                newData.push(data[j]);
            }
        }
        ...
        if (cc.winSize.width > 1280 && SdkUtil.isLiuHaiPin()){
        ...
        for(;i<newData.length;i++){
            ...
            //     }
            // }
            var card = new CSMahjong(MJAI.getDisplayVo(this.direct,1),newData[i]);
    ...
    refreshP2:function(data){
        ...
        switch (this.direct) {
            case 1:
                ...
                initVal = 15 - (cc.winSize.width - 1280)/2;
                if (totalCount >= 14)
                    initVal = (w - (totalCount * g) - (data.length - 1) * 10) / 2;
                if(MJRoomModel.wanfa == MJWanfaType.TJMJ && MJRoomModel.tjmjXJ_isShow){
                    initVal = 250;
                }
                break;
            case 2:
        ...
        for(var i=0;i<data.length;i++) {
            ...
            for(var j=0;j<innerArray.length;j++){
                ...
                if(count<nowCards){
                    ...
                    card = new CSMahjong(MJAI.getDisplayVo(this.direct,2),innerVo);
                    this.mPanel.addChild(card);
                }
                if(innerAction == MJAction.PENG && innerObject.huxi > 0 && j==1){
                    card.showPengDir(innerObject.huxi);
                }
                if(MJRoomModel.wanfa == MJWanfaType.GDCSMJ && innerAction == MJAction.GANG && innerObject.huxi > 0 && j==1){
                    card.showPengDir(innerObject.huxi);
                }
                ...
                if(this.direct==2){
                    ...
                    card.y = initVal-count*g-18*mod + 23;
                }else if(this.direct==3) {
                    card.y = 0;
                } else {
                    card.y = 0;
                ...
                if(gangVo && j==1){
                    if(!card.getChildByTag(333)){
                        ...
                        //    gangVo.a = 1;
                        var gang = new CSMahjong(MJAI.getDisplayVo(this.direct,2),gangVo);
                        if(this.direct==1)
                            gang.y = 20;
                        if(this.direct==2)
                            gang.y += 15;
    ...
    refreshP4: function(data) {
        ...
        for(var i=0;i<data.length;i++) {
            ...
            vo.huDisplay = 1;
            var card = new CSMahjong(MJAI.getDisplayVo(this.direct,4),vo);
            card.x = initVal+g*i;
            card.y = 0;
            if(this.direct==1){
                card.x = initVal-i*g;
    ...
    onShowDesktopSameCards:function(card){
        ...
        if(MJRoomModel.wanfa == MJWanfaType.GDCSMJ){//手牌中相同的也要标记
            for (var i = 0; i < this.mahjongs1.length; i++) {
                var cardVo = this.mahjongs1[i]._cardVo;
                if (cardVo.i == card.i && card.c != cardVo.c) {//去掉点起来的那张牌
                    this.mahjongs1[i].displayGreyBg(true);
                } else {
                    this.mahjongs1[i].removeGreyBg();
                }
            }
        }
    },
    ...
    onRemoveLastSameCards:function(){
        ...
        if(MJRoomModel.wanfa == MJWanfaType.GDCSMJ){//手牌中相同的也要去掉
            for (var i = 0; i < this.mahjongs1.length; i++) {
                this.mahjongs1[i].removeGreyBg();
            }
        }
