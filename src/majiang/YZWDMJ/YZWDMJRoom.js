 ...
 */
var YZWDMJRoom = BaseRoom.extend({
    layouts:{},
    initCoords:{},
    COUNT_DOWN:15,
    ...
    selfRender:function(){
        ...
        this.hbtns = [];
        // cc.log("MJRoomModel.rensh =",MJRoomModel.renshu);
        for(var i=1;i<=6;i++){
        ...
        // if (MJRoomModel.renshu == 2 || MJRoomModel.renshu == 3){
            // this.getWidget("oPanel1").x =   (cc.winSize.width - 1280)/2 + 1280 - disXForIphoneX;
            this.getWidget("Panel_rightButton").x = this.getWidget("Panel_rightButton").x + (cc.winSize.width - 1280)/2 - disXForIphoneX;
            this.getWidget("Panel_leftButton").x =  this.getWidget("Panel_leftButton").x - (cc.winSize.width - 1280)/2 + disXForIphoneX;
        // }
        ...
        //this.recordBtn.setTexture("res/ui/mj/yzwdmjRoom/btn_yuyin_normal.png");
        this.recordBtn.scale = 0.73;
        this.recordBtn.x = 1241 + (cc.winSize.width - 1280)/2 - disXForIphoneX ;
        // //this.recordBtn.y = this.Button_53.y;
        ...
        var gameNameImg = new cc.Sprite("res/ui/mj/yzwdmjRoom/yz.png");
        var x = 640;
        var y = 500;
    ...
    setRadioBtnImg:function(){
        this.audioBtnImg = "res/ui/mj/yzwdmjRoom/btn_yuyin_normal.png";
        this.btnUntouchImg = "res/ui/mj/yzwdmjRoom/btn_yuyin_normal.png";
    },
    ...
    showWanFaImg:function(){
        ...
        // var height = 80+(result.length-1)*65*1.2;
    ...
    onShowHuPanel:function(isShowAllTime){
        ...
        // }
        // var prefix =
        // MJRoomEffects.normalAction(this.root,prefix,this.getWidget("cp"+1));
        // return

        var huList = MJRoomModel.huCards;
        // cc.log("huList =",huList);
        // this.removeHuPanel();
        if (huList && huList.length > 0){
            ...
            for (var i = 0; i < huList.length; i++) {
                ...
                var card = new YZWDMahjong(MJAI.getDisplayVo(1, 2), vo);
    ...
    getMahjongNumById:function(vo){
        ...
        if(vo.i == 201 && MJRoomModel.intParams[31]){
            num = 8;//八红中选项
        }
    ...
    onShowHuCardsByTingPai:function(event){
        ...
        if(huCards.length>0) {
            ...
            for (var i = 0; i < huCards.length; i++) {
                var vo = huCards[i];
                var card = new YZWDMahjong(MJAI.getDisplayVo(1, 4), vo);
                card.x = orderNum * 27;
                card.y = 0;
    ...
    onCheckResult:function(){
        if(this.resultData){
            var mc = new YZWDMJSmallResultPop(this.resultData);
            PopupManager.addPopup(mc);
        }
    },
    ...
    onJixuFromResult:function(){
        this.Panel_8.visible = false;
        if(MJRoomModel.totalBurCount == MJRoomModel.nowBurCount || ClosingInfoModel.ext[12]==1){
            var mc = new YZWDMJBigResultPop(this.resultData);
            PopupManager.addPopup(mc);
        }else {
    ...
    onOver:function(event){
        ...
        this.overTimeout = setTimeout(function(){//延迟弹出结算框
            ...
            var mc = new YZWDMJSmallResultPop(data);
            PopupManager.addPopup(mc);
    ...
    initData:function(){
        ...
        this.roomName_label.setString(MJRoomModel.roomName);
        var wanfa = ClubRecallDetailModel.getSpecificWanfa(MJRoomModel.intParams);
        wanfa = wanfa.replace(/\S*\s\S*\s\S*\s\S*\s/,"");
        wanfa = wanfa.replace(/\s/g,",");
        this.Label_info_mj.setString(wanfa);
        ...
        for(var i=0;i<players.length;i++){
            var p = players[i];
            var seq = MJRoomModel.getPlayerSeq(p.userId,MJRoomModel.mySeat, p.seat);
            var cardPlayer = this._players[p.seat] = new YZWDMJPlayer(p,this.root,seq);
            ...
            if(!isContinue && p.seat == MJRoomModel.mySeat && point >= 0 && MJRoomModel.intParams[10] == 1){
                this.showWaitSelectPiao(true);
            }
            ...
            if(p.userId ==PlayerModel.userId){//自己的状态处理
                ...
                if(MJRoomModel.isOpenTuoguan() && this.bg_CancelTuoguan){
                    ...
                    cc.log("判断是否要显示取消托管按钮... " , isMeTuoguan);
    ...
    showNiaoPanel:function(data){
        ...
        if (birdList && birdList.length > 0){
            for(var i = 0;i < birdList.length; i++) {
                ...
                var vo = MJAI.getMJDef(birdList[i]);
                var card = new YZWDMahjong(MJAI.getDisplayVo(1, 1), vo);
                var size = card.getContentSize();
    ...
    onShowHuCards:function(huArray){
        ...
        if(huArray.length>0){
            ...
            for(var key in huArray){
                var vo = huArray[key];
                //vo.tingDisplay = 1;
                var card = new YZWDMahjong(MJAI.getDisplayVo(1,4),huArray[key]);
                card.x = orderNum*27;
                card.y = 0;
                this.Panel_ting.addChild(card);
                orderNum += 1;
    ...
    displaySelectMahjongs:function(action,resultArray){
        ...
        for(var i=0;i<resultArray.length;i++){
            ...
            for(var j=0;j<chiArr.length;j++){
                ...
                }else if(action==MJAction.XIA_DAN || MJAction.AN_GANG){
                    var danIds = [];
                ...
                var mahjong = new YZWDMahjong(MJAI.getDisplayVo(1,3),chiVo);
                mahjong.scale=scale;
    ...
    onOperate:function(obj,isAlert){
        ...
        switch (temp){
            ...
            case MJAction.BU_ZHANG:
                // cc.log("this.lastLetOutMJ==========",this.lastLetOutMJ)
                var ids = [];
            ...
            case MJAction.GUO:
                ...
                // cc.log("MJRoom::guo click...1",JSON.stringify(MJRoomModel.selfAct));
            ...
            case 1102://永州王钓麻将王钓
                cc.log("永州王钓麻将王闯 王钓...");
                MJRoomModel.sendPlayCardMsg(temp,[]);
                break;
        }
    },
    ...
    refreshButton:function(selfAct){
        ...
        if(selfAct.length>0){
            ...
            this.Panel_btn.visible = true;
            var textureMap = {
                0:{t:"res/ui/mj/yzwdmjRoom/mj_btn_hu.png",v:1},
                1:{t:"res/ui/mj/yzwdmjRoom/mj_btn_peng.png",v:2},
                2:{t:"res/ui/mj/yzwdmjRoom/mj_btn_gang.png",v:3},
                3:{t:"res/ui/mj/yzwdmjRoom/mj_btn_gang.png",v:4},
                4:{t:"res/ui/mj/yzwdmjRoom/mj_btn_chi.png",v:6},
                5:{t:"res/ui/mj/yzwdmjRoom/mj_btn_hu.png",v:1},
                6:{t:"res/ui/mj/yzwdmjRoom/mj_btn_hu_wc.png",v:1101},//王闯
                7:{t:"res/ui/mj/yzwdmjRoom/mj_btn_hu_wd.png",v:1102},//王钓
            };
            var rIndex=0;
            var hasHu = false;
            var btnCount = 0;
            if(selfAct[6] == 1 && selfAct[7]==1){
                textureMap[6] = {t:"res/ui/mj/yzwdmjRoom/mj_btn_hu_wd.png",v:1102};
                textureMap[7] = {t:"res/ui/mj/yzwdmjRoom/mj_btn_hu_wc.png",v:1101};
            }
            ...
            this.hbtns[rIndex].loadTextureNormal("res/ui/mj/yzwdmjRoom/mj_btn_guo.png");
    ...
    onLetOutCard:function(event){
        ...
        switch (action){
            ...
            case 1102:
                ...
                var soundPrefix = (message.zimo==1) ? "zimo" : "hu";;
                if(action == 1101)soundPrefix = "wangchuang";
                if(action == 1102 )soundPrefix = "wangdiao";
                MJRoomSound.actionSound(userId,soundPrefix);
    ...
    smartFitlerMayHuPai:function(isReconect){
        ...
        if(list.length>0){
            ...
            for(var key in list){
                var vo = list[key];
                //vo.tingDisplay = 1;
                var card = new YZWDMahjong(MJAI.getDisplayVo(1,4),list[key]);
                card.x = orderNum*27;
                card.y = 0;
                this.Panel_ting.addChild(card);
                orderNum += 1;
    ...
    onInvite:function(){
        ...
        var youxiName = "永州王钓麻将";
        if(MJRoomModel.tableType == 1){
            youxiName = "[亲友圈]永州王钓麻将"
        }
    ...
    onJoin:function(event){
        var p = event.getUserData();
        var seq = MJRoomModel.getPlayerSeq(p.userId,MJRoomModel.mySeat, p.seat);
        this._players[p.seat] = new YZWDMJPlayer(p,this.root,seq);
        var me = MJRoomModel.getPlayerVo();
        this.btnInvite.visible = (ObjectUtil.size(this._players)<MJRoomModel.renshu);
