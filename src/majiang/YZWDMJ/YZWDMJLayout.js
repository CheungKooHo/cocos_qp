...
var YZWDMJLayout = cc.Class.extend({
	gangPai:[],
    ...
    xiaohu:function(ids){
        ...
        for(var i=0;i<data.length;i++){
            var card = new YZWDMahjong(MJAI.getDisplayVo(this.direct,3),data[i]);
            if(this.direct==1){
                card.x = initVal+i*g;
    ...
    refreshP1:function(data){
        ...
        var correctCoord = function(direct,card,i,zorder,isMopai){
            ...
            if(direct==1){
                ...
                card.x = initVal-i*g*card.scale;
            }else{
                card.x = (direct==4) ? 20 : 0;
            }
            if(direct==4){
                card.y = initVal-i*g*card.scale + 10;
            }else if(direct==2) {
                card.y = initVal+i*g*card.scale;
                card.setLocalZOrder(zorder);
            }else{
                card.y = 0;
            }
            if(newData[i].hasOwnProperty("m") || i>12 || isMopai){//摸牌的处理
                if(direct==1)
                    card.x += 20;
                if(direct==2)
                    card.y += 35 * card.scale;
                if(direct==3)
                    card.x -= 16 * card.scale;
                if(direct==4)
                    card.y -= 35 * card.scale;
            }
            card.initCoord();
        ...
        for(;i<newData.length;i++){
            newData[i].feiDisplay = 1;
            var card = new YZWDMahjong(MJAI.getDisplayVo(this.direct,1),newData[i]);
        ...
        for(var i=0;i<data.length;i++) {
            ...
            for(var j=0;j<innerArray.length;j++){
                ...
                if(count<nowCards){
                    ...
                    card = new YZWDMahjong(MJAI.getDisplayVo(this.direct,2),innerVo);
                    this.mPanel.addChild(card);
                }
                ...
                if(this.direct==2){
                    card.x = -12;
                    card.y = initVal+count*g+18*mod*card.scale;
                    card.setLocalZOrder(zorder);
                }else if(this.direct==4){
                    card.y = initVal-count*g-18*mod*card.scale + 23;
                }else if(this.direct==3) {
                    card.y = 0;
                } else {
                    card.y = 0;
                ...
                if(gangVo && j==1){
                    if(!card.getChildByTag(333)){
                        ...
                        //    gangVo.a = 1;
                        var gang = new YZWDMahjong(MJAI.getDisplayVo(this.direct,2),gangVo);
                        if(this.direct==1)
                            gang.y = 20;
                        if(this.direct==2)
                            gang.y += 15;
    ...
    refreshP4: function(data) {
        ...
        for(var i=0;i<data.length;i++) {
            ...
            vo.huDisplay = 1;
            var card = new YZWDMahjong(MJAI.getDisplayVo(this.direct,4),vo);
            card.x = initVal+g*i;
            card.y = 0;
            if(this.direct==1){
                card.x = initVal-i*g;
