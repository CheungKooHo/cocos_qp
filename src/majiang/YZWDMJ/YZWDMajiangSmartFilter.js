 ...
 */
var YZWDMajiangSmartFilter = cc.Class.extend({
    ...
    isHu:function(handCards, huBean,curIndex){
        ...
        var yzwdMjAI = MJAI.ZZ_CHECK_HU_MJ;
        var length = curIndex>=0? curIndex + MJAI.DEFAULT_CHECK_NUMS : yzwdMjAI.length;
        length = length>yzwdMjAI.length ? yzwdMjAI.length : length;
        var curIndex = curIndex || 0;
        var tingCards = [];
        yzwdMJAILoop:for(var i=curIndex;i<length;i++){
            var cards = [];
            if(handCards.length>0){
                cards = ArrayUtil.clone(handCards);
            }
            cards.push(yzwdMjAI[i]);
            var huBeanCopy = new yzwdMajiangHuBean();
            huBeanCopy.setHuVal(yzwdMjAI[i].i);
            huBeanCopy.setHuId(yzwdMjAI[i].c);
            if(this.judgeHu(cards,huBeanCopy)){
                var isHas = false;
                TingcardsLoop:for(var j=0;j<tingCards.length;j++){
                    if(yzwdMjAI[i].i == tingCards[j].i){
                        isHas = true;
                        break TingcardsLoop;
                    }
                }
                if(!isHas) {
                    tingCards.push(yzwdMjAI[i]);
                }
            }
        }
        return tingCards;
