 ...
 */
var BSMahjong = ccui.Widget.extend({
    ...
    onTouchHandler:function(obj,type){
        if(this.isForSelect()){
            if(type == ccui.Widget.TOUCH_ENDED)
                SyEventManager.dispatchEvent(SyEvent.SELECT_MAJIANG,{action:this._cardVo.se,ids:this._cardVo.ids,isOne:this._cardVo.isOne});
            return false;
        }
        ...
        //    return false;
        if(MJRoomModel.nextSeat == MJRoomModel.banker && MJRoomModel.bsmjCloseCard)
            return false;
