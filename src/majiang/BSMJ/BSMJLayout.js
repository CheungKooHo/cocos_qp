...
var BSMJLayout = cc.Class.extend({
	gangPai:[],
    ...
    changeMahjongRes:function(type){
        cc.log("BSMJ changeMahjongRes =",type);
    ...
    changeMahjongZi:function(type){
        cc.log("BSMJ changeMahjongZi =",type);
        for(var i=0;i<this.mahjongs1.length;i++){
    ...
    xiaohu:function(ids){
        ...
        for(var i=0;i<data.length;i++){
            var card = new BSMahjong(MJAI.getDisplayVo(this.direct,3),data[i]);
            if(this.direct==1){
                card.x = initVal+i*g;
    ...
    refresh:function(data1,data2,data3,data4,bankerSeat,isMoPai){
        cc.log("data2 =",JSON.stringify(data2));
        cc.log("data4 =",JSON.stringify(data4));
    ...
    refreshP1:function(data){
        ...
        //var gapMapping = {1:78,2:34,3:44,4:34};
        ...
        for(;i<newData.length;i++){
            newData[i].feiDisplay = 1;
            var card = new BSMahjong(MJAI.getDisplayVo(this.direct,1),newData[i]);
    ...
    refreshP2:function(data){
        ...
        for(var i=0;i<data.length;i++) {
            ...
            for(var j=0;j<innerArray.length;j++){
                ...
                if(count<nowCards){
                    ...
                    card = new BSMahjong(MJAI.getDisplayVo(this.direct,2),innerVo);
                    this.mPanel.addChild(card);
                }
                ...
                if(gangVo && j==1){
                    if(!card.getChildByTag(333)){
                        ...
                        //    gangVo.a = 1;
                        var gang = new BSMahjong(MJAI.getDisplayVo(this.direct,2),gangVo);
                        if(this.direct==1)
                            gang.y = 20;
                        if(this.direct==2)
                            gang.y += 15;
        ...
        if(this.direct==1)
            ...
            // cc.log("this.p2Mark =",this.p2Mark);
        if(this.direct==2)
    ...
    refreshP4: function(data) {
        ...
        for(var i=0;i<data.length;i++) {
            ...
            vo.huDisplay = 1;
            var card = new BSMahjong(MJAI.getDisplayVo(this.direct,4),vo);
            card.x = initVal+g*i;
            card.y = 0;
            if(this.direct==1){
                card.x = initVal-i*g;
    ...
    onShowDesktopSameCards:function(card){
        cc.log("onShowDesktopSameCards card =",card);
        for (var i = 0; i < this.mahjongs3.length; i++) {
