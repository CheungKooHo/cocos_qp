...
var HZMJReplay = BaseLayer.extend({
    ...
    showDiCard:function(diCardId,kingCardId){
        if(MJReplayModel.wanfa == MJWanfaType.TJMJ){
    ...
    showLiangWangAni:function(id,kingCardId){
        ...
        if(MJReplayModel.wanfa == MJWanfaType.TJMJ){
            ...
            if(MJReplayModel.intParams[25] == 1){/** 八王模式王牌小一点 */
                wangpai.x = 95;
    ...
    initData:function(flag){
        ...
        for(var i=0;i<players.length;i++){
            ...
            if(p.ext && MJReplayModel.wanfa == MJWanfaType.TJMJ){
        ...
        if(step){
            ...
            switch (action){
                ...
                case 20://下蛋
                    ...
                    if(action==7){
                        prefix = "bu";
                        if(MJReplayModel.wanfa == MJWanfaType.YJMJ){
                            prefix = "gang";
                        }
                    }else if(action==6){
                        prefix = "chi";
                ...
                case 11://长沙麻将杠
                    ...
                    if(MJReplayModel.wanfa == MJWanfaType.AHMJ){
                        for (var i = 0; i < ids.length; i++) {
                            this.getLayout(seq).chuPai(MJAI.getMJDef(ids[i]),true);
                        }
                    }else if(MJReplayModel.wanfa == MJWanfaType.TJMJ){
            ...
            if(MJReplayModel.step==(MJReplayModel.steps.length-1)){
                // cc.log("MJReplayModel.wanfa =",MJReplayModel.wanfa);
                var mc = null;
                if(MJReplayModel.wanfa == MJWanfaType.ZZMJ){
                    mc = new ZZMJSmallResultPop(MJReplayModel.closingMsg,true);
                }else if(MJReplayModel.wanfa == MJWanfaType.CSMJ || MJReplayModel.wanfa == MJWanfaType.TDH){
                    mc = new CSMJSmallResultPop(MJReplayModel.closingMsg,true);
                }else if(MJReplayModel.wanfa == MJWanfaType.YZWDMJ){
                    mc = new YZWDMJSmallResultPop(MJReplayModel.closingMsg,true);
                }else if(MJReplayModel.wanfa == MJWanfaType.YJMJ){
                    mc = new YJMJSmallResultPop(MJReplayModel.closingMsg, true);
                }else if(MJReplayModel.wanfa == MJWanfaType.NXMJ){
                    mc = new NXMJSmallResultPop(MJReplayModel.closingMsg,true);
                }else if(MJReplayModel.wanfa == MJWanfaType.DZMJ){
                    mc = new DZMJSmallResultPop(MJReplayModel.closingMsg,true);
                }else{
    ...
    refreshButton:function(seatId,actArray){
        ...
        var textureMap = {//添加 index 用来排序
            0:{t:"res/ui/mj/tjmjRoom/hu.png",v:1,index:10},
            1:{t:"res/ui/mj/tjmjRoom/peng.png",v:2,index:7},
            2:{t:"res/ui/mj/tjmjRoom/gang.png",v:3,index:9},
            3:{t:"res/ui/mj/tjmjRoom/gang.png",v:4,index:9},
            4:{t:"res/ui/mj/tjmjRoom/chi.png",v:6,index:6},
            ...
            17:{t:"res/ui/mj/tjmjRoom/hu.png",v:1,index:10}
        };

        if(MJReplayModel.wanfa == MJWanfaType.YJMJ){
            textureMap[4] = {t:"res/ui/mj/tjmjRoom/gang.png",v:7,index:1};
            textureMap[6] = {t:"res/ui/mj/zzmjRoom/mj_btn_ting.png",v:14,index:1};
        }else if(MJReplayModel.wanfa == MJWanfaType.HZMJ
            || MJReplayModel.wanfa == MJWanfaType.ZZMJ
            || MJReplayModel.wanfa == MJWanfaType.DZMJ){
            textureMap[5] = {t:"res/ui/mj/tjmjRoom/hu.png",v:1,index:10};
        }else if (MJReplayModel.wanfa == MJWanfaType.YZWDMJ){
            textureMap = {
        ...
        buttonList.push({t:"res/ui/mj/tjmjRoom/guo.png",v:1,index:0});
    ...
    showTJMJGang:function(acts){
        if(acts.length > 0){
            ...
            for(var i = 0;i<acts.length;++i){
                ...
                var card = new HZMahjong(MJAI.getDisplayVo(1, 1), MJAI.getMJDef(id));
                card.x = i * 105 + 10;
                card.y = 5;
