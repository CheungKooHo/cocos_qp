 ...
 */
var YJMJBigResultPop = BasePopup.extend({
    user:null,
    ctor: function (data,isDaiKai) {
        this.data = data;
        this.isDaiKai = isDaiKai;
        this._super("res/HZmjBigResult.json");
    },
    ...
    refreshSingle:function(widget,user){
        ...
        var zmCount = user.zimoCount || 0;
        var jpCount = user.jiePaoCount || 0;
        var fpCount = user.fangPaoCount || 0;


        var dahuName = ["碰碰胡","将将胡","清一色","七小对","豪华七小对","双豪华七小对","三豪华七小对","杠爆","抢杠胡","海底捞",
            "一条龙","门清","天胡","一字撬","报听","码码胡"];

        var typeData = [];
        typeData.push({name:"自摸次数",num:zmCount});
        typeData.push({name:"接炮次数",num:jpCount});
        typeData.push({name:"放炮次数",num:fpCount});

        var arr = user.dahuCounts;
        for(var i = 0;i<arr.length;++i){
            if(arr[i] > 0){
                typeData.push({name:dahuName[i],num:arr[i]});
            }
        }

        ccui.helper.seekWidgetByName(widget,"Label_78").visible = false;
        ccui.helper.seekWidgetByName(widget,"Label_78_0").visible = false;
        ccui.helper.seekWidgetByName(widget,"Label_78_1").visible = false;
        ccui.helper.seekWidgetByName(widget,"Label_78_2").visible = false;

        ccui.helper.seekWidgetByName(widget,"Label_zm").visible = false;
        ccui.helper.seekWidgetByName(widget,"Label_zm_0").visible = false;
        ccui.helper.seekWidgetByName(widget,"Label_zm_1").visible = false;
        ccui.helper.seekWidgetByName(widget,"Label_zm_2").visible = false;

        var parent = ccui.helper.seekWidgetByName(widget,"Label_78").getParent();
        ...
        for(var i = 0;i<typeData.length;++i){
            ...
            var label_name = new cc.LabelTTF(typeData[i].name,"Arial",24);
            label_name.setColor(cc.color(254,217,144));
            label_name.setPosition(100,item.height/2);
            item.addChild(label_name);

            var label_num = new cc.LabelTTF(String(typeData[i].num),"Arial",24);
            label_num.setColor(cc.color(254,217,144));
    ...
    selfRender: function () {
        ...
        if (MJRoomModel.roomName){
            var string = "";
            string = MJRoomModel.roomName + "   亲友圈ID：" + ClosingInfoModel.ext[15];
        ...
        this.getWidget("wanfa_2").setString("沅江麻将");
        ...
        var wanfaStr  = ClubRecallDetailModel.getYJMJWanfa(MJRoomModel.intParams);
    ...
    qyqStartAnother:function(){
        ...
        var groupId = ClosingInfoModel.ext[15];
        var modeId = 0;
