 ...
 */
var DTZMoneyBigResultPop = BasePopup.extend({
    user: null,
    ctor: function (data,isDaiKai) {
        ...
        if(DTZRoomModel.isMoneyRoom()){
            this.isMoneyRoom = true;
            json = "res/DTZMoney3RenBigResult.json";
        }else{
            this.isMoneyRoom = false;
        }
    ...
    dealMoneyView:function(){
        if(this.isMoneyRoom){
            this.getWidget("LabelRoomCard").setString("娱乐场");
            this.getWidget("Label_RoomId").setString(this.getMoneyBeilv() + "倍");
            this.getWidget("Label_Desc").setString("房间服务费：" + this.getMoneyCost());
        }
    },
    ...
    refreshTeamData: function (widget, user, teamId) {
        ...
        if(this.isMoneyRoom){//金币场的牌面分要额外减去上下游分数
        ...
        if (this.is4Ren()) {
            ...
            //显示奖券
            if (user.ext[22] && parseInt(user.ext[22]) > 0 && PlayerModel.userId == user.userId){
                winloseScoreLabel.y = winloseScoreLabel.y + 15;

                var img = "res/ui/dtz/giftExChangePop/img_3.png";
                var spriteBg = new cc.Sprite(img);
                spriteBg.x = 75;
                spriteBg.y = -35;
                winloseScoreLabel.addChild(spriteBg);
                ...
                var lqlabel = new cc.LabelTTF("+"+user.ext[22], "Arial", 28);
                lqlabel.y = 15;
                lqlabel.x = 50;
    ...
    loadData:function(teamDataStruct  , dateSource){
        ...
        teamDataStruct.ext = dateSource.ext;
        teamDataStruct.userId = dateSource.userId;
    },
