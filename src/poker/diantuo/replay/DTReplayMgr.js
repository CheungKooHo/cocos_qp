...
var DTReplayMgr = {
    ...
    handleReplayData:function(hfData){
        ...
        this.msgArr.push({type:DTTabelType.CreateTable,data:data});
            ...
            if(step.action == 0 || step.action == 1){//出牌，不要
                ...
                for (var i = 0; i < table.players.length; ++i) {
                    ...
                    if (p.seat == msg.data.seat) {
                        p.handCardIds = DTRoomModel.delCardWithArr(p.handCardIds, msg.data.cardIds);
                        ...
                        if(msg.data.isLet > 0){
                            if(table.renshu == 2){
                                p.ext[1] -= msg.data.isLet;
                            }else{
                                p.ext[1] -= msg.data.isLet/3;
                            }
                        }
                    }
                }
                ...
                msg.type = DTTabelType.XuanDuZhan;
                msg.data = {};
                ...
                msg.type = DTTabelType.ChangeTuoGuan;
                msg.data = {};
    ...
    runReplay:function(hfData){
        ...
        if(this.msgArr.length > 0){
            ...
            var ctrLayer = new DTReplayCtrLayer();
            ...
            DTRoomModel.init(table);
            layer.handleTableData(DTTabelType.CreateTable,table);
    ...
    sendTableMsg:function(step){
        ...
        DTRoomModel.init(table);
        SyEventManager.dispatchTableEvent(DTTabelType.CreateTable,table);
    },
    ...
    sendOverMsg:function(){
        SyEventManager.dispatchTableEvent(DTTabelType.OnOver,this.resultData);
    },
