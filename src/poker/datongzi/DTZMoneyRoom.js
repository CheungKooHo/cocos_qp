 ...
 */
var DTZMoneyRoom = BaseRoom.extend({
...
//原Room.js 中的代码
    ...
    onTouchCardPanel:function(obj,type) {
        if (type == ccui.Widget.TOUCH_BEGAN){
            DTZCardDelegate.dealTouchBegin(this , obj , type);
    ...
    dealMoneyView:function(){
        var isMoneyRoom = DTZRoomModel.isMoneyRoom();
        if(isMoneyRoom){
            cc.log("当前是金币场房间！！！！");
            ...
            this.tempLable3.setString("娱乐场");
            this.lableRoomId.setString(DTZRoomModel.getMoneyRoomBeilv() +"倍");
    ...
    onOver: function (event) {
        ...
        this.showResultTimeOutHandle = setTimeout(function () {//延迟弹出结算框
            ...
            if(DTZRoomModel.isMoneyRoom()){//直接显示大结算界面
                //var mc = new DTZBigResultPop(data);
                var mc = new DTZMoneyBigResultPop(data);
    ...
    onJoin: function (event) {
        ...
        if(DTZRoomModel.isMoneyRoom() && ObjectUtil.size(this._players) >= DTZRoomModel.renshu){
    ...
    showProgressOfTuoguan:function(seat){
        var coords = {1:{x:300,y:310},2:{x:985,y:530},3:{x:290,y:530}};
        var progressTime = this._countDown = DTZRoomModel.getTuoguanTime();
        cc.log("ProgressTime::" , DTZRoomModel.getTuoguanTime());
        if(DTZRoomModel.isOpenTuoguan){
            //测试圆形时间进度条
            if(!this.coolProgress){
                this.coolProgress = new CoolProgress(
            ...
            this.coolProgress.beginProgress(progressTime ,100 , 0);
        }
