 ...
 */
var SDHPlayerLayer = cc.Layer.extend({
    ctor:function(){
        this._super();
    ...
    onUpdateClubTableCoin:function(event){
        ...
        for(var i = 0; i<data.length; i++){
            ...
            var p = SDHRoomModel.getPlayerData(userId);
    ...
    handleTableData:function(type,data){
        if(type == SDHTabelType.CreateTable){
            this.players[2].setVisible(SDHRoomModel.renshu == 4);
            ...
            var seq = SDHRoomModel.getSeqWithSeat(params[2]);

            this.players[seq - 1] && this.players[seq - 1].setJiaoFenStr(params[0]);

            if(params[3] > 0){
                //清理掉当前叫分人的上轮叫分
                var seq = SDHRoomModel.getSeqWithSeat(params[3]);
                this.players[seq - 1] && this.players[seq - 1].setJiaoFenStr();
            }


        }else if(type == SDHTabelType.DingZhuang){
            for(var i = 0;i<SDHRoomModel.players.length;++i){
                var p = SDHRoomModel.players[i];
                var seq = SDHRoomModel.getSeqWithSeat(p.seat);
        ...
        for(var i = 0;i<players.length;++i){
            for(var j = 0;j<players.length;++j){
                if(i != j && players[i].ip == players[j].ip && players[i].ip){
                    var seq = SDHRoomModel.getSeqWithSeat(players[i].seat);
    ...
    onMagicAction:function(event){
        ...
        var from_p = SDHRoomModel.getPlayerData(userId);
        var from_seat = SDHRoomModel.getSeqWithSeat(from_p.seat);
        var to_seat = SDHRoomModel.getSeqWithSeat(seat);
...
var SDHPlayerNode = cc.Node.extend({
    playerData:null,
    ...
    setPlayerWithData:function(data){
        ...
        if(SDHRoomModel.getIsSwitchCoin()){
        ...
        this.icon_zhuang.setVisible(data.seat == SDHRoomModel.banker);
    ...
    setJiaoFenStr:function(data){
        var show_my = SDHRoomModel.nextSeat != this.playerData.seat;
        if(data){
            this.playerData.ext[1] = data;
            show_my = true;
        }else{
            data = this.playerData.ext[1]
        }

        var jiaofen = "";
        if(SDHRoomModel.remain == 1 && show_my){
            if(data == 0)jiaofen = "bj";
            else if(data > 0){
                jiaofen = data;

                if(SDHRoomModel.ext[4] == 1){
                    jiaofen += " p";
                }
            }
        }
        this.label_JiaoFen.setString(jiaofen);
    },
