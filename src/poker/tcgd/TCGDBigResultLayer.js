 ...
 */
var TCGDBigResultLayer = cc.Layer.extend({
    ...
    setLayerWithData:function(){
        ...
        this.label_credit_config.setString(TCGDRoomModel.getCreditConfigStr());
    ...
    addResultItem:function(players){
        ...
        for(var i = 0;i<players.length;++i){
            var item = new TCGDBigResultItem();
    ...
    setInfoData:function(){
        ...
        var jushu = "局数:" + TCGDRoomModel.nowBurCount;
        if(TCGDRoomModel.totalBurCount < 100){
            jushu += ("/" + TCGDRoomModel.totalBurCount);
        }
    ...
    onShare:function(){
        ...
        var obj={};
        var tableId = TCGDRoomModel.tableId;
        obj.tableId=tableId;
...
var TCGDBigResultItem = cc.Node.extend({
    ctor:function(){
        this._super();
    ...
    setItemWithData:function(data,isFangZhu,isDyj){
        ...
        this.labelArr1[1].setString(data.actionCounts[2] || 0);//一游次数

        var scoreArr = [];
        scoreArr.push(data.totalPoint || 0);//积分

        if(TCGDRoomModel.isCreditRoom()){
