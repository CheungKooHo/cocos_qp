...
var PDKRoom = BaseRoom.extend({//Room
	...
	initData:function(){
		...
		var hongshiDesc = PDKRoomModel.getWanfaString();
	...
	showDaNiaoState:function(type){
		...
		if(type == 1 || type == 2){
			...
			if(type == 1){
				var btn1 = new ccui.Button("res/ui/mj/bsmjRoom/btn_dn.png");
				btn1.setPosition(150,0);
				btn1.flag = 1;
				UITools.addClickEvent(btn1,this,this.onClickDaNiaoBtn);
				daNiaoNode.addChild(btn1);

				var btn2 = new ccui.Button("res/ui/mj/bsmjRoom/btn_bdn.png");
				btn2.setPosition(-150,0);
				btn2.flag = 0;
				UITools.addClickEvent(btn2,this,this.onClickDaNiaoBtn);
				daNiaoNode.addChild(btn2);
			}else if(type == 2){
				var tipLabel = new cc.LabelTTF("等待其他玩家选择打鸟","Arial",36);
	...
	selfRender:function(){
        ...
        this.Panel_huikan.x = 800;
        this.Panel_huikan.y = 430;
        this.Panel_huikan.width = 600;
        this.Panel_huikan.height = 300;
    ...
    showPiaoFenPanel:function(type) {
        ...
        if (type==3){//飘1/2/3
            ...
            this.Button_p1f.visible = false;
            this.Button_p2f.x = 365;
            this.Button_p5f.x = 595;
            this.Button_p3f.visible = false;
            this.Button_p5f.visible = true;
            this.Button_p8f.visible = true;
        }
    },
	...
	showBgColor: function (_type) {
		//绿色背景1蓝色背景2紫色背景3
		// cc.log("_type =",_type);
		var bgTexture = "res/ui/bbt/Bgc2.jpg";
		if (_type == 1){
			bgTexture = "res/ui/bbt/Bgc1.jpg";
	...
	onLetOutCard:function(event){
		...
		if(message.cardIds.length==0){//要不起
			...
			// var seq = this.getPlayerSeq(-1,PDKRoomModel.mySeat,this.seatSeq[seat][1]);
		...
		if(nextSeat == PDKRoomModel.mySeat){//轮到我出牌了
			if(message.isLet===1){
				...
				if(result){
					...
					// 	var curGame = PDKRoomModel.nowBurCount;//当前局数
					// 	setTimeout(function(){
					...
					// 		}
					// 		// if (curGame && curGame == PDKRoomModel.nowBurCount){
					// 		// 	self.onPlayCard(self.btnBreak,true);
					// 		// }
					// 	},300);
					// }else{
						this.smartLetOut();
	...
	onPlayCard:function(obj,bombBreak){
		// cc.log("this._cCardPattern.type =",JSON.stringify(this._cCardPattern));
		// cc.log("this._allCards =",JSON.stringify(this._allCards));
		// this._allCards = [{t:2,n:13,i:13,c:213}]
		var self = this;
		if (!this.isHasHei3Frist()){
			FloatLabelUtil.comText("开房第一局出的牌必须包含黑桃三");
			return;
		}
		// cc.log("this._allCards =",JSON.stringify(this._allCards));
		// cc.log("this.getCardsOnHand() =",JSON.stringify(this.getCardsOnHand()));
	...
	OutCardsSelf:function(){
		...
		var cardIds = [];
		if (this._cCardPattern.type == AI.THREE || this._cCardPattern.type == AI.PLANE || this._cCardPattern.type == AI.BOMBWITHCARD){
		...
		this.showJianTou(nextSeat);
		RoomSound.letOutSound(PlayerModel.userId,this._cCardPattern);
		RoomEffects.play(this.root,this._cCardPattern);
	},
	...
	startGame:function(event){
		...
		this.Button_17.visible = this.Button_30.visible = this.Image_baodan.visible =false;
