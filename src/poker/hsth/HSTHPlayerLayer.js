 ...
 */
var HSTHPlayerLayer = cc.Layer.extend({
    ctor:function(){
        this._super();
    ...
    onUpdateClubTableCoin:function(event){
        ...
        for(var i = 0; i<data.length; i++){
            ...
            var p = HSTHRoomModel.getPlayerData(userId);
            var localSeat = HSTHRoomModel.getSeqWithSeat(p.seat);
    ...
    handleTableData:function(type,data){
        if(type == HSTHTabelType.CreateTable){
            ...
            var seq = HSTHRoomModel.getSeqWithSeat(seat);
            ...
            for(var i = 0;i<HSTHRoomModel.players.length;++i){
                var p = HSTHRoomModel.players[i];
                var seq = HSTHRoomModel.getSeqWithSeat(p.seat);
        ...
        for(var i = 0;i<players.length;++i){
            for(var j = 0;j<players.length;++j){
                if(i != j && players[i].ip == players[j].ip && players[i].ip){
                    var seq = HSTHRoomModel.getSeqWithSeat(players[i].seat);
    ...
    onFastChat:function(event){
        ...
        var p = HSTHRoomModel.getPlayerData(data.userId);
        var localSeat = HSTHRoomModel.getSeqWithSeat(p.seat);
    ...
    onMagicAction:function(event){
        ...
        var from_p = HSTHRoomModel.getPlayerData(userId);
        var from_seat = HSTHRoomModel.getSeqWithSeat(from_p.seat);
        var to_seat = HSTHRoomModel.getSeqWithSeat(seat);
...
var HSTHPlayerNode = cc.Node.extend({
    playerData:null,
    ...
    setPlayerWithData:function(data){
        ...
        this.icon_zhuang.setVisible(data.seat == HSTHRoomModel.banker);
    ...
    setRemainCards:function(data){
        this.playerData = data;
        //看剩余牌数
        if(!HSTHRoomModel.replay && this.idx > 0
            && HSTHRoomModel.remain > 0 && data.ext[8] > 0){
        ...
        if(data.ext[8] > 0 && data.ext[8] < 5){
            ...
            if(HSTHRoomModel.intParams[3] != 1){
                this.paiBei.setVisible(false);
            }
        }
    },
    ...
    setUserScoreWithData:function(data){
        ...
        this.label_ext_score.setString(data.ext[11] || 0);
    ...
    initNode:function(){
        ...
        var tip1 = new cc.Sprite("res/ui/hsth/zongfen.png");
        ...
        var tip2 = new cc.Sprite("res/ui/hsth/xifen.png");
        ...
        this.label_score.setPosition(this.fenbg1.width/2 + 30,this.fenbg1.height/2);
        this.fenbg1.addChild(this.label_score);

        this.label_ext_score = new cc.LabelTTF("123","Arial",22);
        this.label_ext_score.setPosition(this.fenbg2.width/2 + 30,this.fenbg2.height/2);
        this.fenbg2.addChild(this.label_ext_score);
    ...
    setPosWithIdx:function(idx){
        ...
        switch (idx) {
            case 0:
                this.label_name.setPosition(0,55);

                this.fenbg1.setPosition(100,5);
                this.fenbg2.setPosition(100,-26);

                this.label_sai.setAnchorPoint(0,0.5);
                this.label_sai.setPosition(40,35);

                this.label_coin.setPosition(65,35);
                this.image_coin.setPosition(50,35)
